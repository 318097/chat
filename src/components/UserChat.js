...
const UserChat = ({ history }) => {
  const [chat, setChat] = useState([
    { message: 'hi', date: '2019-1-1', _id: '32' },
    { message: 'hi dsfs sdf sd sdf', date: '2019-1-1', _id: '323232' },
    { message: 'hi sdfjlk jsdof jsdof jdsofkdsfkl ', date: '2019-1-1', _id: '3232', user: 'me' },
    ...
    <section id="chat">
      ...
      <div className="chat-container">
        {chat.map(item => (
          <div
            key={item._id}
            style={{ display: 'flex', justifyContent: item.user === 'me' ? 'flex-end' : 'flex-start' }}
          >
            <Message data={item} />
          </div>
        ))}
